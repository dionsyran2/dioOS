...
window_t* CreateWindow(uint32_t style, wchar_t* ClassName, wchar_t* title, uint32_t rsv, int x, int y, uint32_t width, uint32_t height, window_t* parent, void* resv, thread_t* thread){
    wchar_t* cn = new wchar_t[strlen(ClassName)];
    wchar_t* tl = new wchar_t[strlen(title)];
    memcpy(cn, ClassName, strlen(ClassName) * sizeof(wchar_t));
    memcpy(tl, title, strlen(title) * sizeof(wchar_t));

    cn[strlen(ClassName)] = L'\0';
    tl[strlen(title)] = L'\0';
...
void DispatchMessage(MSG* msg){
    if (msg == nullptr || msg->wc == nullptr) return; 
...
int DefWindowProc(window_t* win, uint32_t msg, uint64_t param1, uint64_t param2){
    switch (msg)
    {
        ...
        case WM_DESTROY:{
            WNDCLASS* wc = (WNDCLASS*)param1;
            PostQuitMessage(wc, 0);
            return 0;
        }
...
void DrawCircle(uint8_t* buffer, int pitch, int centerX, int centerY, int radius, uint32_t color, bool fill) {
    for (int y = -radius; y <= radius; y++) {
        for (int x = -radius; x <= radius; x++) {
            ...
            if ((fill && distSq <= rSq) || (!fill && distSq >= rSq - radius && distSq <= rSq + radius)) {
                int px = centerX + x;
                int py = centerY + y;
...
CreateWidgetStruct::CreateWidgetStruct(window_t* parent, WIDGET_TYPE_EX type, const wchar_t* text, int identifier, int x, int y, uint32_t width, uint32_t height, int style){
    this->parent = parent;
    this->type = type;
    
    int txt_len = strlen(text);
    wchar_t* txt = new wchar_t[txt_len + 1];
    strcpy(txt, (wchar_t*)text);
...
WIDGETEX* CreateWidgetEx(window_t* parent, WIDGET_TYPE_EX type, const wchar_t* text, int identifier, int x, int y, uint32_t width, uint32_t height, int style){
    CreateWidgetStruct* st = new CreateWidgetStruct(parent, type, text, identifier, x, y, width, height, style);
    WIDGETEX* w = CreateWidgetAsm(st);
