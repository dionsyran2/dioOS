...
EFI_STATUS SetGraphicsMode(UINT32 width, UINT32 height) {
    EFI_GRAPHICS_OUTPUT_PROTOCOL* gop;
    ...
    for (UINTN i = 0; i < max_mode; i++) {
        ...
        if (info->HorizontalResolution == width && info->VerticalResolution == height) {
            ...
            if (EFI_ERROR(status)) {
                Print(L"Unable to set mode %u x %u\n", width, height);
                return status;
            }
            Print(L"Resolution set to %u x %u\n", width, height);
            return EFI_SUCCESS;
        }
    }

    Print(L"Requested resolution not available\n");
    return EFI_UNSUPPORTED;
}

EFI_STATUS efi_main (EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
	InitializeLib(ImageHandle, SystemTable);
	SetGraphicsMode(1920, 1080);
	...
	PSF1_FONT* newFont = LoadPSF1Font(NULL, L"zap-ext-vga16.psf", ImageHandle, SystemTable);
	if (newFont == NULL){
