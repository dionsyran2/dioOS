...
namespace WM{
    ...
    button_t* startBtn = nullptr;
    button_t* time_button = nullptr;
    ...
    void CreateTaskbarEntry(window_t* win){
        //win->taskbar_btn = taskbar->createButton(0, 0, 0, 0, L"");
        //win->taskbar_btn->param = (uint64_t)win;
    ...
    void RemoveTaskbarEntry(window_t* win){
        ...
        //taskbar->removeButton(win->taskbar_btn);
    ...
    void RefreshTime(){
        ...
        if (time_button == nullptr){
            time_button = taskbar->createButton(width, height, sx, sy, L"");
    ...
    void InitTaskbar(thread_t* task){
        ...
        startBtn = taskbar->createButton(taskbar->width * 0.042, taskbar->height - 6, 2, 4, L"Start");
    ...
    void TaskbarHandleBtn(window_t* win, button_t* btn){
        if (btn == startBtn){
            StartCallback(win);
        }else if (btn == time_button){
            time_btn_cb();
        }else {
            taskbar_entry_btn_cb(win, (window_t*)btn->param);
        }
    }
}