...
namespace WM
{
    ...
    widget_t* focusedWidget = nullptr;

    void focus(widget_t* widget){
        if (focusedWidget){
...
#define INCREMENT 12345
    ...
    bool widget_t::Clicked(){
        ...
        if ((style & WS_DISABLED) == 0 && parent->_Focus == 1 && mouseX >= xStart && mouseX <= xEnd && mouseY >= yStart && mouseY <= yEnd && prevMouse1Down == 0 && mouse1Down == 1)
        {
            return true;
        }
        return false;
    ...
    widget_t* window_t::CreateWidget(WIDGET_TYPE type, wchar_t* text, uint32_t style, int x, int y, int width, int height, uint64_t id, uint64_t extra){
    ...
    widget_t* window_t::CreateWidget(WIDGET_TYPE type, wchar_t* text, uint32_t style, int x, int y, int width, int height, uint64_t id){
        CreateWidget(type, text, style, x, y, width, height, id, NULL);
    }

    void window_t::RemoveWidget(widget_t* widget){
        int index = -1;
        ...
        for (int i = index; i < num_of_widgets; i++){
            widgets[i] = widgets[i + 1];
        }

        num_of_widgets--;
        widgets[num_of_widgets] = nullptr;
    }
    ...
    void widget_t::Render(){
        ...
        switch(type){
            ...
            case WIDGET_TYPE::Label:{
                /* Render the background */
                uint32_t backgr = style & WS_NOBACKGROUND;
                uint8_t border = style & WS_NOBORDER ? 0 : 2;
                if (backgr == 0){
                    for (uint32_t py = y; py < y + height; py++)
                    {
                ...
                uint32_t tx = x + (width - textWidth) / 2;
                uint32_t ty = y + (height - txtHeight) / 2;
                fontRenderer->print(extra, text, tx, ty, parent->width, parent->width, parent->buffer, txtHeight);
                break;
            }
            ...
            case WIDGET_TYPE::TextBox:{
                ...
                bool use_txt = text != nullptr;
                if (use_txt) use_txt = use_txt && strlen(text) > 0;
                uint8_t textSize = 16;
                uint8_t textWidth = WM::fontRenderer->getTextLength(use_txt ? text : placeHolder, fontSize);
                
                if (textWidth > width - (border * 2)){
                    fontSize /= 3/4;
                    return Render();
                }
                uint32_t tx = x/* + (width - textWidth) / 2*/;
                uint32_t ty = y + (height - fontSize) / 2;
                cursor_y = (height - fontSize) / 2;
                cursor_x = use_txt ? textWidth + 1 : 1;
                fontRenderer->print(use_txt ? extra : 0x808080, use_txt ? text : placeHolder, tx, ty, parent->width, parent->width, parent->buffer, fontSize);
                break;
            }
    ...
    void window_t::removeButton(widget_t* btn){
        if (!btn) return;
    ...
    void window_t::refreshTitleBar()
    {
        ...
        if (closeBtn == nullptr){
            closeBtn = CreateWidget(WIDGET_TYPE::Button, L"X", 0, btnx, btny, btn_width, btn_height, UINT16_MAX);
        }
    ...
    void ProcessKeypress(uint8_t scancode){
        ...
        //Handle any textboxes

        if (focusedWidget != nullptr){
            switch(focusedWidget->type){
                case WIDGET_TYPE::TextBox:{
                    if (focusedWidget->text == nullptr){
                        focusedWidget->text = new wchar_t[100];
                        focusedWidget->text[0] = L'\0';
                    }else{
                        /*wchar_t* newText = new wchar_t[strlen(focusedWidget->text)];
                        strcpy(newText, focusedWidget->text);
                        newText[strlen(focusedWidget->text)] = L'\0';
                        free(focusedWidget->text);
                        focusedWidget->text = newText;*/
                    }

                    wchar_t ascii = QWERTYKeyboard::Translate(scancode, SHIFT || CAPS, NUMLOCK);
                    wchar_t str[2] = {ascii, L'\0'};
                    if (ascii == 0) return;
                    strcat(focusedWidget->text, str);
    ...
    void ErrorPopup(wchar_t* error){
        ...
        widget_t* btn = win->CreateWidget(WIDGET_TYPE::Button, L"OK", 0, 100, 20, (400 - 100) / 2, 100 - 20 - 10, 1);
        btn->Render();
    }
    ...
    void blipCursorTask(){
        widget_t* widget = focusedWidget;
        uint32_t background_before[widget->height] = { 0 };
        uint32_t background_after[widget->height] = { 0 };

        //widget->cursor_x = widget->text != nullptr ? fontRenderer->getTextLength(widget->text, widget->fontSize) : 0;
        /*while(widget->cursor_x > widget->width){
            widget->cursor_x -= widget->width;
            widget->cursor_y += widget->fontSize;
        }*/

        while(focusedWidget == widget){
            for (int y = 0; y < widget->fontSize; y++){
                background_before[y] = widget->parent->getPix(widget->x + widget->cursor_x, widget->y + y + widget->cursor_y);
            }

            for (int y = 0; y < widget->fontSize; y++){
                widget->parent->setPix(widget->x + widget->cursor_x, widget->y + y + widget->cursor_y, 0x0000FF);
            }

            for (int y = 0; y < widget->fontSize; y++){
                background_after[y] = widget->parent->getPix(widget->x + widget->cursor_x, widget->y + y + widget->cursor_y);
            }

            widget->parent->dirty_rects[widget->parent->amount_of_dirty_rects].x = widget->x;
            widget->parent->dirty_rects[widget->parent->amount_of_dirty_rects].y = widget->y;
            widget->parent->dirty_rects[widget->parent->amount_of_dirty_rects].width = widget->width;
            widget->parent->dirty_rects[widget->parent->amount_of_dirty_rects].height = widget->height;
            widget->parent->amount_of_dirty_rects++;
            widget->parent->_Refresh = true;
            Sleep(500);

            for (int y = 0; y < widget->fontSize; y++){
                if (widget->parent->getPix(widget->x + widget->cursor_x, widget->y + y + widget->cursor_y) == background_after[y]){
                    widget->parent->setPix(widget->x + widget->cursor_x, widget->y + y + widget->cursor_y, background_before[y]);
                }
            }
    ...
    void LOOP(){
        while(1){
            uint64_t startTicks = GetSystemTicks();
                    ...
                    if (prevMouse1Down == 0)
                    {
                        ...
                        else
                        {
                            for (int i = 0; i < window->num_of_widgets; i++)
                            {
                                ...
                                if (widget->type == WIDGET_TYPE::TextBox && widget->Clicked()){
                                    if (widget == focusedWidget) continue;
                                    focus(widget);
                                    CreateThread((void*)blipCursorTask, GetThread());
                                }
                                else if (widget->type == WIDGET_TYPE::Button && widget->Clicked())
                                {
                                    if (widget == window->closeBtn){ //Send window close msg
                                        MSG* msg = new MSG;
                                        msg->win = window;
                                        msg->code = WM_CLOSE;
                                        msg->wc = window->wc;
                                        ...
                                        msg->wc = window->wc;
                                        msg->param1 = widget->menuID;
                                        msg->param2 = (uint64_t)widget;
                                        SendMessage(msg);
                                    }
            ...
            const wchar_t* str = toWString((uint64_t)fps);
            uint64_t width = fb->common.framebuffer_width;
            for (int y = 0; y < 30; y++){
                for (int x = width - 50; x < width; x++){
                    PutPixInBB(x, y, 0);
                }
            }
            fontRenderer->print(0xFFFFFF, str, width - fontRenderer->getTextLength((wchar_t*)str, 20) - 10, 10, width, fb->common.framebuffer_pitch / (fb->common.framebuffer_bpp / 8), (void*)backbuffer, 20);
            memcpy((void*)fb->common.framebuffer_addr, backbuffer, sizeOfFB); // Slow but will work

            uint64_t ticksToDrawFrame = GetSystemTicks() - startTicks + 1; // + 1 incase it finished in less than 1ms
            fps = 1000 / ticksToDrawFrame; // 1 tick = 1 ms.
        }
    }
}
#endif