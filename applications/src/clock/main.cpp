...
void Update(){
    ...
    r *= 0.9;
    float angle = (2.0f * M_PI / 60.0f) * time->second - M_PI / 2.0f;  // i = 0 to 11
    float sec_x = cx + r * cos(angle);
    float sec_y = cy + r * sin(angle);

    r *= 0.8;
    angle = (2.0f * M_PI / 60.0f) * time->minute - M_PI / 2.0f;  // i = 0 to 11
    float min_x = cx + r * cos(angle);
    float min_y = cy + r * sin(angle);

    r *= 0.8;
    angle = (2.0f * M_PI / 12.0f) * time->hour - M_PI / 2.0f;  // i = 0 to 11
    float h_x = cx + r * cos(angle);
    float h_y = cy + r * sin(angle);
...
extern "C" void _main(thread_t* thread)
{
    ...
    uint32_t width = 200;
    uint32_t height = 250;
    int x = (fb->common.framebuffer_width - width) / 2;
    int y = ((fb->common.framebuffer_height - height) / 2) - 30;

    WNDCLASS* cl = new WNDCLASS;
    memset(cl, 0, sizeof(WNDCLASS));

    wchar_t* cn = new wchar_t[10];
    memcpy(cn, L"Clock", sizeof(wchar_t) * 6); //as we do not yet support reading memory of other processes, this will have to do
    ...
    thread_t* sub = CreateThread((void*)loop, thread);
    strcpy(sub->task_name, "Clock");
    MSG* msg = new MSG;
