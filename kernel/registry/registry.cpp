#include "registry.h"
#include "../syscalls/syscalls.h" // yep... totally not dodgy... not at all
char* reg_file_name = "C:\\sys_registry.sys"; //its just a txt file
...
void CreateKeyFromFileEntry(char* path, char* entry){
    ...
    value_length = entry_length - name_length - type_length - 4;
    ...
    memcpy(value, entry + name_length + type_length + 4, value_length);
...
int LoadRegistry(){
    DirEntry* file_entry = VolumeSpace::OpenFile((char*)reg_file_name);
    if (!file_entry) return -1;

    char* file = (char*)file_entry->volume->LoadFile(file_entry);
    if (file == nullptr) return -2;

    LoadEntries(file);

    //free(file);
    VolumeSpace::CloseFile(file_entry);
    return 0;
}
        ...
        if (strcmp(keyPath, key->path) == 0 && strcmp(valueName, key->name) == 0){
            switch(key->type){
                ...
                case REG_KEY_TYPE::REG_STRING:
                    char* d = new char[strlen(key->str_value)];
