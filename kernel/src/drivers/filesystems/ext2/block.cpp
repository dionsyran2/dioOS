...
namespace filesystems{
    ...
    int64_t ext2_t::_allocate_block(uint32_t start_group) {
        ...
        for (int i = 0; i < 2; i++) {
            for (uint32_t indx = current_start; indx < current_end; indx++) {
                ...
                // Read Bitmap
                if (disk->read(bitmap, this->block_size, _block_to_offset(group->block_of_block_usage_bitmap)) < 0) {
                    free(bitmap);
                ...
                for (uint64_t b = 0; b < blocks_per_group; b++) {
                    ...
                    if ((*bitmask & (1 << (b % 32))) == 0) {
                        ...
                        *bitmask |= (1 << (b % 32));
                        ...
                        if (disk->write(bitmap, this->block_size, _block_to_offset(group->block_of_block_usage_bitmap)) < 0) {
                            free(bitmap);
                            spin_unlock(&this->block_group_table_lock, rflags);
                            return -EIO;
                        }
    ...
    perform_allocation:
        ...
        if (disk->read(bitmap, this->block_size, _block_to_offset(target_group->block_of_block_usage_bitmap)) < 0) {
            free(bitmap); spin_unlock(&this->block_group_table_lock, rflags); return -EIO;
        }
        ...
        // Write Back
        if (disk->write(bitmap, this->block_size, _block_to_offset(target_group->block_of_block_usage_bitmap)) < 0) {
            free(bitmap); spin_unlock(&this->block_group_table_lock, rflags); return -EIO;
        }
