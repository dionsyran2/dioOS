...
namespace filesystems {
    int64_t ext2_t::_allocate_inode(uint32_t start_group) {
        ...
        for (int i = 0; i < 2; i++) {
            for (uint32_t indx = current_start; indx < current_end; indx++) {
                ...
                // Read Bitmap
                if (disk->read(bitmap, this->block_size, _block_to_offset(group->block_of_inode_usage_bitmap)) < 0) {
                    free(bitmap);
                ...
                for (uint64_t b = 0; b < inodes_per_group; b++) {
                    ...
                    if ((*bitmask & (1 << (b % 32))) == 0) {
                        ...
                        *bitmask |= (1 << (b % 32));
                        ...
                        if (disk->write(bitmap, this->block_size, _block_to_offset(group->block_of_inode_usage_bitmap)) < 0) {
                            free(bitmap);
                            spin_unlock(&this->block_group_table_lock, rflags);
                            return -EIO;
                        }
    ...
    ext2_inode_t* ext2_t::fetch_inode(uint32_t inode) {
        ...
        if (disk->read(node, to_read, offset) < 0) {
            delete node;
            return nullptr;
        }
    ...
    void ext2_t::update_inode(uint32_t inode, ext2_inode_t* inode_struct) {
        ...
        disk->write(inode_struct, to_write, offset);
    }
    ...
    uint32_t ext2_t::_create_indirect_block(uint32_t parent_group) {
        ...
        disk->write(zeros, this->block_size, _block_to_offset(block));
        free(zeros);
    ...
    uint32_t ext2_t::_read_indirect_entry(uint32_t block_id, uint32_t index){
        ...
        bool ret = disk->read(block, this->block_size, offset); // TODO Handle Failure
     ...
     bool ext2_t::_set_indirect_entry(uint32_t block_id, uint32_t index, uint32_t value){
        ...
        bool ret = disk->read(block, this->block_size, offset);

        block[index] = value;

        ret = disk->write(block, this->block_size, offset);
        free(block);
    ...
    int64_t ext2_t::load_inode_contents(uint32_t inode_num, uint64_t offset, uint64_t size, void* buffer) {
        ...
        while (total_read < total_to_read) {
            ...
            if (disk->read((char*)buffer + total_read, actual_read_size, disk_offset) < 0) {
                delete inode; 
                return -EIO;
            }
    ...
    int64_t ext2_t::write_inode_contents(uint32_t inode_num, uint64_t offset, uint64_t size, const void* buffer) {
        ...
        while (total_written < size) {
            ...
            // Perform the Write
            if (disk->write((const char*)buffer + total_written, to_write, phys_offset) < 0) {
    ...
    int64_t ext2_t::truncate_inode(uint32_t inode_num, uint64_t new_size) {
        ext2_inode_t* inode = fetch_inode(inode_num);
        if (!inode) return -ENOENT;
        ...
        if (boundary_limit % this->block_size != 0) {
            ...
            if (phys != 0) {
                ...
                disk->read(buf, this->block_size, _block_to_offset(phys));
                
                // Zero everything after the valid data
                memset(buf + offset_in_blk, 0, this->block_size - offset_in_blk);
                
                disk->write(buf, this->block_size, _block_to_offset(phys));
                free(buf);
            }
        }
        ...
        if (new_size < old_size) {
            ...
            for (uint32_t logical_idx = old_block_count; logical_idx < new_block_count; logical_idx++) {
                ...
                if (disk->write(zero_buf, this->block_size, _block_to_offset(phys_block)) < 0) {
                    _free_block(phys_block);
                    free(zero_buf);
                    delete inode;
                    return -EIO;
    ...
    int ext2_t::free_inode_contents(uint32_t inode){
        ...
        if ((node->mode & S_IFMT) == S_IFLNK && get_inode_size(node) <= 60) {
