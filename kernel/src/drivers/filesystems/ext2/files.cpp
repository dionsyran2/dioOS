...
namespace filesystems {
    int ext2_t::create_file(uint32_t parent_inode_num, const char* name, uint16_t mode) {
        ...
        new_inode->mode = S_IFREG | (mode & (~S_IFMT));
        ...
        if (this->extended_superblock.file_block_preallocation_count){
            truncate_inode(new_inode_num, this->extended_superblock.file_block_preallocation_count * this->block_size);
        }

        delete new_inode;

        // Link into Parent
        directory_add_entry(parent_inode_num, new_inode_num, name, S_IFREG);
