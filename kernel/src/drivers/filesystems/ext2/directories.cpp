...
namespace filesystems {
            ...
            while (internal_offset < this->block_size) {
                ...
                if (entry->entry_size != real_used_size && internal_offset + entry->entry_size >= this->block_size) {
                    ...
                    if (free_space >= required_size) {
                        ...
                        if (this->extended_superblock.required_features & EXT2_REQ_FEATURE_DIR_TYPE) new_entry->type = (type == S_IFDIR) ? 2 : 1;
    ...
    int ext2_t::create_directory(uint32_t parent_inode_num, const char* name, uint16_t mode) {
        ...
        new_inode->mode = S_IFDIR | (mode & (~S_IFMT));
        ...
        // Link into Parent
        directory_add_entry(parent_inode_num, new_inode_num, name, S_IFDIR);
