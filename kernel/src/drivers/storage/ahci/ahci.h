...
namespace AHCI {
    ...
    class Port{
        public:
            ...
            bool ATAPICMD(uint8_t[12], void* DBA, size_t bytecnt);
            bool EjectDisk();
            bool SetSpeedPI(uint8_t speed);
            bool ATAPI_READ_TOC(void* memory);
            uint8_t* GetMediumSize();
            bool ReadPI(uint64_t sector, uint32_t sectorCount, void* buffer);
    }__attribute__((packed));;
