...
namespace AHCI{
    ...
    PortType CheckPortType(HBAPort* port){
        ...
        while(i < 10000 || deviceDetection == 1 || deviceDetection == 2 || (port->signature == 0xFFFFFFFF && deviceDetection != 0)){
            ...
            if (deviceDetection == 3 && port->signature != 0xFFFFFFFF) {  // 3 = Device detected & communicating
                globalRenderer->printf("Device detected!\n");
                break;
            }
            i++;
        }

        globalRenderer->printf("DEV_P: %hh | IPM: %hh | sig: %x\n", deviceDetection, interfacePowerManagement, port->signature);*/
    ...
    AHCIDriver::AHCIDriver(PCI::PCIDeviceHeader* pciBaseAddress){
        ...
        //ABAR->globalHostControl |= 1;
        //while(ABAR->globalHostControl & 0b01);
        /*
        ...
        */
        ...
        //Set the controller to AHCI mode
        ABAR->globalHostControl |= 1 << 31;
        //ABAR->globalHostControl |= 1 << 1; //Int Enable

        volatile uint32_t* cap2 = (volatile uint32_t*)((uint8_t*)ABAR + 0x24); // CAP2 register
        volatile uint32_t* bos = (volatile uint32_t*)((uint8_t*)ABAR + 0x28); // BIOS OS handoff

        if (*cap2 & (1 << 0)) {  // If BOH (BIOS-OS Handoff) is supported
            globalRenderer->print("Bios Handoff supported\n");
            if (*bos & (1 << 4)) {  // BIOS owns the controller
                *bos |= (1 << 1);    // Request OS control
                while (*bos & (1 << 4)); // Wait until BIOS releases control
            }
        }
        ...
        ProbePorts();
        uint32_t portsImpemented = ABAR->portsImplemented;
        //globalRenderer->printf("PI: %d | PS: %d\n", portsImpemented, ABAR->hostCapability & 0xF);
        
        for (int i = 0; i < portCount; i++){
            globalRenderer->printf("Port: %d\n", i);
            Port* port = Ports[i];
            ...
            if (port->portType == PortType::SATAPI){
                DISK_DRIVE* dsk = registerDisk((void*)port, AHCI_ATAPI);
                continue;
                uint32_t* data = (uint32_t*)port->GetMediumSize();
                uint32_t totalLba = *data;
                port->SetSpeedPI(1);
                void* buff = GlobalAllocator.RequestPage();
                port->ATAPI_READ_TOC(buff);
                if (!port->ATAPI_READ_TOC(buff)) return;
                uint8_t* tocData = (uint8_t*)buff;
                uint16_t tocLength = (tocData[0] << 8) | tocData[1];
                uint8_t firstTrack = tocData[2];
                uint8_t lastTrack = tocData[3];

                globalRenderer->printf("First Track: %d\n", firstTrack);
                globalRenderer->printf("Last Track: %d\n", lastTrack);

                for (int i = 0; i < (tocLength - 2) / 8; i++) {
                    uint8_t trackNum = tocData[4 + i * 8 + 2];
                    uint32_t trackLBA = (tocData[4 + i * 8 + 4] << 24) |
                                        (tocData[4 + i * 8 + 5] << 16) |
                                        (tocData[4 + i * 8 + 6] << 8) |
                                        (tocData[4 + i * 8 + 7]);
                
                    uint32_t nextTrackLBA;
                    if (i < (tocLength - 2) / 8 - 1) { // Not the last track
                        nextTrackLBA = (tocData[4 + (i + 1) * 8 + 4] << 24) |
                                       (tocData[4 + (i + 1) * 8 + 5] << 16) |
                                       (tocData[4 + (i + 1) * 8 + 6] << 8) |
                                       (tocData[4 + (i + 1) * 8 + 7]);
                    } else { // Last track: use the total CD length
                        nextTrackLBA = totalLba; 
                    }
                
                    uint32_t trackLength = nextTrackLBA - trackLBA;
                    
                    uint32_t minutes = trackLength / (75 * 60);
                    uint32_t seconds = (trackLength / 75) % 60;
                    uint32_t frames  = trackLength % 75;
                
                    globalRenderer->printf("Track %d starts at LBA %d, length: %d:%d:%d\n",
                                           trackNum, trackLBA, minutes, seconds, frames);
                }
