...
int HDA::RefillBuffer()
{
    ...
    if ((*Stream & 0b10) != 0b10){
       //kprintf("Stream not running\n");
        while(1);
    }
   //kprintf(globalRenderer->randHexColor(), ".");
    for (int i = nextchunk; i < nextchunk + 32; i++)
    {
        ...
        memcpy((void *)bd->address, (void *)((uint64_t)buff + offset), 128);
        offset += 128;
    }
    asm("wbinvd");
    nextchunk += 32;
...
void HDA::Play(uint8_t *data, size_t sz, uint32_t frequency, uint32_t bps, uint32_t channels)
{
   ...
   //kprintf("Stream Reset!\n");
    ...
    for (int i = 0; i < 256; i++)
    {
        ...
        if (bd->address & 0x7F) {
           //kprintf("Not aligned to 128 bytes!\n");
        }
