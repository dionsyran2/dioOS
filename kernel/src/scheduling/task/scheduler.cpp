...
namespace taskScheduler{
    ...
    void task_t::initialize(void* function){
        ...
        this->stack = (uint64_t)GlobalAllocator.RequestPages(10); //40 kb
        this->kernelStack = (uint64_t)GlobalAllocator.RequestPages(10); //40 kb
        this->state = Stopped;
        this->valid = true;
        this->resetCounter();
    ...
    PageTableManager* task_t::CreatePageTableMgr(){
        ...
        memcpy(ptm->PML4, globalPTM.PML4, sizeof(PageTable));
        return ptm;
    }
    ...
    extern "C" void RunCurrentTask(){
        ...
        if (currentTask->state == Paused || currentTask->state == Blocked){ 
            ...
            if (currentTask->userspace){
                ...
                asm volatile ("sti");
                asm volatile ("mov %0, %%rsp" :: "r" (currentTask->stack + (0x1000 * 10)));
