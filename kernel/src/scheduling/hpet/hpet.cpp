...
namespace HPET
{
    ...
    void Initialize(){
        ...
        *GCReg &= ~1;
        ...
        ticksIn1MS = f / 1000;
        globalRenderer->printf("TICKS: %d / %d\n", ticksIn1MS, hdr->minimum_tick);
        ...
        *GCReg |= 0b11;
        ...
        globalRenderer->printf("[HPET] Waiting for an interrupt\n");
        while(ticks == 0){
            globalRenderer->printf(globalRenderer->randHexColor(), "%d", *counter);
            __asm__ volatile ("hlt");
        }
        globalRenderer->printf("[HPET] Interrupt received!\n");
