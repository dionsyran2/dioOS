...
void checkFATValues(uint64_t entry) {
    if (entry == 0x00000000) {
        globalRenderer->print("Cluster is free/");
    } else if (entry >= 0x0FFFFFF8 && entry <= 0x0FFFFFFF) {
        globalRenderer->print("End of file chain/");
    } else if (entry == 0x0FFFFFF7) {
        globalRenderer->print("Bad cluster/");
    } else if (entry >= 0x0FFFFFF0 && entry <= 0x0FFFFFF6) {
        globalRenderer->print("Reserved cluster/");
    } else {
        globalRenderer->print("Next cluster in chain: ");
        globalRenderer->print(toString(entry));
...
DirEntries* FAT32::ListDir(uint8_t* dir){
    ...
    //globalRenderer->printf("rootclustersector: %d\n", rootClusterSector);
    uint32_t clusterSize = sectorsPerCluster * bytesPerSector;
...
uint8_t* FAT32::LoadFile(uint8_t* DirEntry){
    ...
    //globalRenderer->printf("SECTORS PER CLUSTERS %d\n", sectorsPerCluster);
...
void FAT32::Init(DISK_DRIVE* port_, uint64_t off) {
    ...
    uint32_t numOfFats = *((uint32_t*)(bpb + 0x10));
    uint16_t bytesPerSector = *((uint32_t*)(bpb + 0x0B));
    //globalRenderer->printf("Reserved Sectors: %d, Sectors Per FAT: %d, Number Of Fats: %d, bps: %d\n", reservedSectors, sectorsPerFAT, numOfFats, bytesPerSector);
    ...
    //globalRenderer->printf(0xFF0000, "1");
    port->Read(offset + fatStartSector, sectorsPerFAT, fatBuffer);
    //globalRenderer->printf(0xFF0000, "2");
    uint32_t rootCluster = *((uint32_t*)(bpb + 0x02C));
    uint8_t sectorsPerCluster = *((uint32_t*)(bpb + 0x0D));
    ...
    //globalRenderer->printf(0xFFFF00, "%d - %d - %d - %d - %d - %d", rootClusterSector + offset, numOfFats, sectorsPerFAT, rootCluster, sectorsPerCluster, offset);
    port->Read(rootClusterSector + offset, sectorsPerCluster, rootDir);
    //globalRenderer->printf(0xFF0000, "4");
