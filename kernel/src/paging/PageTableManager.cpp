...
void PageTableManager::ClonePTM(PageTableManager* ptm){
    for (int PDP_i = 0; PDP_i < 512; PDP_i++){
        PageDirectoryEntry* PDPEntry = &PML4->entries[PDP_i];
        if (!PDPEntry->GetFlag(PT_Flag::Present)) continue;

        PageTable* PDP = (PageTable*)PDPEntry->GetAddress();

        for (int PD_i = 0; PD_i < 512; PD_i++){
            PageDirectoryEntry* PDEntry = &PDP->entries[PD_i];
            if (!PDEntry->GetFlag(PT_Flag::Present)) continue;

            PageTable* PD = (PageTable*)PDEntry->GetAddress();

            for (int PT_i = 0; PT_i < 512; PT_i++){
                PageDirectoryEntry* PTEntry = &PD->entries[PT_i];
                if (!PTEntry->GetFlag(PT_Flag::Present)) continue;

                PageTable* PT = (PageTable*)PTEntry->GetAddress();
                
                for (int P_i = 0; P_i < 512; P_i++){
                    PageDirectoryEntry* P = &PT->entries[P_i];
                    if (!P->GetFlag(PT_Flag::Present)) continue;

                    uint64_t virtualAddress = 0;
                    virtualAddress |= ((uint64_t)PDP_i & 0x1FF) << 39;
                    virtualAddress |= ((uint64_t)PD_i  & 0x1FF) << 30;
                    virtualAddress |= ((uint64_t)PT_i  & 0x1FF) << 21;
                    virtualAddress |= ((uint64_t)P_i   & 0x1FF) << 12;
                    
                    ptm->MapMemory((void*)virtualAddress, (void*)P->GetAddress());
                }
