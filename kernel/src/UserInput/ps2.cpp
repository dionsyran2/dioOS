...
namespace PS2{
    ...
    int EnablePort(uint8_t port, bool interrupts){
        ...
        if (port == 1){
            ...
            globalRenderer->printf("ENABLING: %hh\n", config);
    ...
    int Initialize(){
        ...
        if (res != PS2_SELF_TEST_PASS){
            globalRenderer->printf(0xFF0000, "[PS2] ");
            globalRenderer->printf("Self test failed (res = %hh)\n", res);
            return -1;
        }
        ...
        if (res != PS2_PORT_TEST_PASS){
            globalRenderer->printf(0xFF0000, "[PS2] ");
            globalRenderer->printf("Primary port test failed (res = %hh)\n", res);
            return -1;
        }
        ...
        if (dualPort){
            ...
            if (res != PS2_PORT_TEST_PASS){
                globalRenderer->printf(0xFF0000, "[PS2] ");
                globalRenderer->printf("Secondary port test failed (res = %hh)\n", res);
                return -1;
            }
        }
        
        globalRenderer->printf(0x00FF00, "[PS2] ");
        globalRenderer->printf("The controller has been initialized successfully!\n", res);
    }
}